...
#endif
	Partition += "/Z_Simulations_Nov2022/InputData/";
	if (nome) {
		Partition += nome;
		Partition += "/";
	}
	...
	do {
		...
		if (chardata == '@') {
			...
			case 26:
				...
				break;
			case 27:
				simulationdata.AdaptPercent = data;
				break;
			case 28:
...
void SaveUAndFluxErrors(TPZAnalysis *an,TPZVec<REAL> &erro,SimulationData &simulationdata,std::ofstream &saida) {
	an->SetExact(simulationdata.Exact);
...
TPZStepSolver<STATE> *SetStructuralMatrixAndLinearEquationSolver(TPZAnalysis *an, TPZCompMesh *cmesh, SimulationData &simulationdata, int nthreads) {
	...
	if(!simulationdata.MatrixType)
		matstruct = new TPZSkylineStructMatrix(cmesh);
	else if(simulationdata.MatrixType == 1)
		matstruct = new TPZSpStructMatrix(cmesh);
//		matstruct = new TPZSkylineNSymStructMatrix(cmesh);
	else if(simulationdata.MatrixType == 2)
		matstruct = new TPZSBandStructMatrix(cmesh);
	else if(simulationdata.MatrixType == 3)
		matstruct = new TPZBandStructMatrix(cmesh);
	
	if (nthreads) matstruct->SetNumThreads(nthreads);
...
int PutNameDirectoryForResults(std::string& sout) {
	...
	sout.erase(tt + 9);
#endif
	sout += "/Z_Simulations_Nov2022/SimulationResults/R";
...
void ComputingUhAndGradUhOnCenterOfTheElement(TPZCompEl *cel, TPZVec<REAL> &qsi, TPZVec<STATE> &Sol, TPZFMatrix<REAL> &gradXY) {
	if (!cel) return;
	TPZMaterialData dataT;
	cel->Reference()->CenterPoint(cel->Reference()->NSides() - 1, qsi);
//	((TPZInterpolatedElement*)cel)->ComputeSolution(qsi, dataT, 0);
	TPZSolVec sol;
	TPZGradSolVec grad;
	TPZFMatrix<REAL> axes;

	cel->Reference()->CenterPoint(cel->Reference()->NSides() - 1, qsi);
	cel->ComputeSolution(qsi, sol, grad, axes);
	TPZAxesTools<REAL>::Axes2XYZ(grad[0], gradXY, axes);
	Sol = sol[0];
//	gradXY = dataT.dphix[0];
//	Sol = dataT.sol[0];
}
