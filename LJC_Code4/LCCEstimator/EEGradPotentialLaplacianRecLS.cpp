...
TEEGradPotentialLaplacianRecLS::TEEGradPotentialLaplacianRecLS(TPZCompMesh *cmesh, SimulationData simul) : TErrorEstimator(cmesh, simul) {
}
TEEGradPotentialLaplacianRecLS::~TEEGradPotentialLaplacianRecLS() {
}

/* Function as error estimator for each element */
REAL TEEGradPotentialLaplacianRecLS::RecoveredGradient(TPZCompEl *cel) {
...
int TEEGradPotentialLaplacianRecLS::ComputingEstimationsByElement(TPZCompEl *cel, TPZVec<REAL> &Estimatives) {
	...
	for (int ineighs = 0; ineighs < nneighs; ineighs++) {
		...
		for (j = 0; j < dim; j++) {
			...
			H(ineighs, j + dim) = 0.5*H(ineighs, j)*H(ineighs, j);
		}
		H(ineighs, 2 * dim) = H(ineighs, 0)*H(ineighs, 1);
		if (dim == 3) {
			H(ineighs, 8) = H(ineighs, 0)*H(ineighs, 2);
			H(ineighs, 9) = H(ineighs, 1)*H(ineighs, 2);
		}
