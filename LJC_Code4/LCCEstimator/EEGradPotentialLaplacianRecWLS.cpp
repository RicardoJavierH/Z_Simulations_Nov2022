...
TEEGradPotentialLaplacianRecWLS::TEEGradPotentialLaplacianRecWLS(TPZCompMesh *cmesh, SimulationData simul) : TErrorEstimator(cmesh, simul) {
}
TEEGradPotentialLaplacianRecWLS::~TEEGradPotentialLaplacianRecWLS() {
}

/* Function as error estimator for each element */
REAL TEEGradPotentialLaplacianRecWLS::RecoveredGradient(TPZCompEl *cel) {
...
int TEEGradPotentialLaplacianRecWLS::ComputingEstimationsByElement(TPZCompEl *cel, TPZVec<REAL> &Estimatives) {
	...
	for (int ineighs = 0; ineighs < nneighs; ineighs++) {
		...
		F(ineighs, 0) = weight * (solbeta[fVariable]);
	}
