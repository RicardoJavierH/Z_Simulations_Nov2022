...
TPZGeoMesh *CreatingGeometricMeshFromGIDFile(std::string &GeoGridFile, SimulationData &simul, int &DimProblem) {
	...
	if(!simul.nMatId) return 0;
	SetMaterialIdForElements(gmesh, simul.vMatId[0]);

	// With only one boundary condition (Dirichlet with value ZERO.
	if(!simul.nBCMatId) return 0;
	SetMatBCIdForElementsSameByHeigh(gmesh, simul.vBCMatId[0], 'a');
	return gmesh;

	// EXAMPLE TO USE INITIAL AND LAST POINTS TO DETERMINE BC
	TPZManVector<REAL> P0(3, 0.0L);
	TPZManVector<REAL> P1(3, 0.0L);
...
int ChooseEquation(SimulationData &simul, int Dim) {
	...
	if (!Equation) {
		...
		if (Dim == 3) {
			dums = new TPZDummyFunction<STATE>(FforcingSolin, 5);
			dum = new TPZDummyFunction<STATE>(ExactSolin, 5);
			simul.Exact = &ExactSolin;   //    nfunc = 9;
		}
	}
	else if (Equation == 4) {
		if (Dim == 3) {
			dums = new TPZDummyFunction<STATE>(FforcingRachowicz, 5);
			dum = new TPZDummyFunction<STATE>(ExactRachowicz, 5);
			simul.Exact = &ExactRachowicz;   //    nfunc = 9;
		}
	}
	else if (simul.Equation == 5) {
		if (Dim == 3) {
			if (simul.NumericMethod == 2)
				dums = new TPZDummyFunction<STATE>(MinusSourceFunctionLinear3D, 5);
			else
				dums = new TPZDummyFunction<STATE>(SourceFunctionLinear3D, 5);
			dum = new TPZDummyFunction<STATE>(SolExactLinear3D, 5);
			simul.Exact = &SolExactLinear3D;
		}
		else if (Dim == 2) {
			if (simul.NumericMethod == 2)
				dums = new TPZDummyFunction<STATE>(MinusSourceFunctionLinear2D, 5);
			else
				dums = new TPZDummyFunction<STATE>(SourceFunctionLinear2D, 5);
			dum = new TPZDummyFunction<STATE>(SolExactLinear2D, 5);
			simul.Exact = &SolExactLinear2D;
		}
		else {
			if (simul.NumericMethod == 2)
				dums = new TPZDummyFunction<STATE>(MinusSourceFunctionLinear1D, 5);
			else
				dums = new TPZDummyFunction<STATE>(SourceFunctionLinear1D, 5);
			dum = new TPZDummyFunction<STATE>(SolExactLinear1D, 5);
...
TPZMaterial *CreatingComputationalMesh(TPZCompMesh* cMesh, SimulationData &simul, int DimProblem) {
	/// criar materiais
	TPZMaterial *mat = 0;
	if(simul.timeDependent) {
		/// criar materiais
		TLCCMatPoissonTime *Mater;
		if (!simul.nMatId) return mat;
		Mater = new TLCCMatPoissonTime(simul.vMatId[0], DimProblem);
		Mater->SetStepTime(simul.DeltaT);
		TPZVec<REAL> Con(2, 0.);
		Mater->SetParameters(0.1037, 0., Con); // mudar K (cm2/min)
	//	Mater->SetParameters(0.1729089737L,0,Con); // mudar K
		Mater->SetForcingFunction(simul.SourceFunc);
		Mater->SetForcingFunctionExact(simul.solExata);
		mat = (TPZMaterial*)Mater;
	}
	else {
		TPZMatPoisson3d *Mater;
		if (!simul.nMatId) return mat;
		Mater = new TPZMatPoisson3d(simul.vMatId[0], DimProblem);
		//	TPZMatLaplacian* mat = new TPZMatLaplacian(elementIdMat, DimProblem);
		const STATE K = 1., F = 0.;
		TPZVec<REAL> Conv(2, 0.);
		Mater->SetParameters(K, F,Conv);
		mat = (TPZMaterial *)Mater;
	}
	mat->SetForcingFunction(simul.SourceFunc);
	mat->SetForcingFunctionExact(simul.solExata);

	if (simul.NumericMethod==1 && simul.timeDependent) {
		///Formulacao nao-simetria de Baumann, Oden e Babuska sem penalizacao
		((TLCCMatPoissonTime*)mat)->SetNoPenalty();
		((TLCCMatPoissonTime*)mat)->SetNonSymmetric();
	}
	cMesh->InsertMaterialObject(mat);

	///Condições de contorno
	//mat->SetForcingFunction();
	TPZFMatrix<STATE> val1(1, 1, simul.BoundaryValue[0]), val2(1, 1, simul.BoundaryValue[0]);
	
	TPZBndCond* BCondDirichletNulo = mat->CreateBC(mat, simul.vBCMatId[0], 0, val1, val2);//0 = Dirichlet
	if(simul.Equation==3) {
		// To Fichera corners - Dirichlet
		TPZAutoPointer<TPZFunction<STATE> > FunctionBC = new TPZDummyFunction<STATE>(BCSolin, 5);
		val1.PutVal(0,0,1.);
		//	val1.PutVal(1,1,1.);
		//	val1.PutVal(2,2,1.);
		((TPZMaterial *)BCondDirichletNulo)->SetForcingFunction(FunctionBC);
	}
	...
	TPZMaterial *bndN = mat->CreateBC(mat,-41,1,val1,val2);
	...
	TPZMaterial *bndD = mat->CreateBC(mat,-40,0,val1,val2);
...
#include "TPZVecL2.h"
 void CreatingFluxMesh(TPZCompMesh* cmesh,SimulationData	&simul, int DimProblem) {
	 TPZVecL2 *mat;
	mat = new TPZVecL2(simul.vMatId[0]);
	mat->SetDimension(DimProblem);

	cmesh->InsertMaterialObject(mat);
	TPZFNMatrix<1, STATE> val1(1, 1, 0.0L), val2(1, 1, 1.0L);
...
//	TPZFMatrix<REAL> InvK(DimProblem+1, DimProblem+1, 0.0L);
	...
	TPZFMatrix<STATE> val1(1, 1, 0.0L), val2(1, 1, 0.0L);

	TPZMaterial * BCond0 = material->CreateBC(mat, simul.vBCMatId[0], 0, val1, val2);
