...
TErrorEstimator * TAdaptive::AllocatingEstimator(TPZCompMesh *cmesh, SimulationData simuldata) {
	...
	if (!methodestimator) {
		if (typeestimator != 0 && typeestimator != 1) return 0;
		TEEGradientRecLS* adestimator = new TEEGradientRecLS(cmesh, simuldata);
		return (TErrorEstimator *)adestimator;
	}
	else if (methodestimator == 1) {
		if (typeestimator != 0 && typeestimator != 1) return 0;
		return (TErrorEstimator *)(new TEEGradientRecWLS(cmesh, simuldata));
	}
	else if (methodestimator == 2) {
	...
	else if (methodestimator == 4) {
		if (typeestimator != 0 && typeestimator != 1 && typeestimator != 2 && typeestimator != 4 && typeestimator != 8) return 0;
		return (TErrorEstimator *)(new TEEGradPotentialRecLS(cmesh, simuldata));
	}
	else if (methodestimator == 5) {
		if (typeestimator != 0 && typeestimator != 1 && typeestimator != 2 && typeestimator != 4 && typeestimator != 8) return 0;
		return (TErrorEstimator *)(new TEEGradPotentialRecWLS(cmesh, simuldata));
	}
	else if (methodestimator == 6) {
		if (typeestimator != 2 && typeestimator != 3 && typeestimator != 6 && typeestimator != 8) return 0;
		return (TErrorEstimator *)(new TEELaplacianPotentialRecLS(cmesh, simuldata));
	}
	else if (methodestimator == 7) {
		if (typeestimator != 2 && typeestimator != 3 && typeestimator != 6 && typeestimator != 8) return 0;
		return (TErrorEstimator *)(new TEELaplacianPotentialRecWLS(cmesh, simuldata));
	}
	else if (methodestimator == 8) {
		if (typeestimator != 0 && typeestimator != 1 && typeestimator != 3 && typeestimator !=5 && typeestimator < 9) return 0;
		return (TErrorEstimator *)(new TEEGradLaplacianRecLS(cmesh, simuldata));
	}
	else if (methodestimator == 9) {
		if (typeestimator != 0 && typeestimator != 1 && typeestimator != 3 && typeestimator != 5 && typeestimator < 9) return 0;
		return (TErrorEstimator *)(new TEEGradLaplacianRecWLS(cmesh, simuldata));
	}
	else if (methodestimator == 10) {
		return (TErrorEstimator *)(new TEEGradPotentialLaplacianRecLS(cmesh, simuldata));
	}
	else if (methodestimator == 11) {
		return (TErrorEstimator *)(new TEEGradPotentialLaplacianRecWLS(cmesh, simuldata));
	}
	else
		return 0;
}
...
void TAdaptive::ClassifyingCelsByEstimations(TErrorEstimator *estimator,std::string &dir) {
	...
	bool printinfo = false, check = false;
	if (printinfo)
...
void TAdaptive::Adapting(TErrorEstimator *estimator,std::ostream &out,std::string &dir) {
	...
	int64_t counter = 0, nelemsamedim = CheckElementsSameDimension(fCMesh);
	if(fElements2_HP_PM.NElements())
		counter = UniformHRefineSelectionWithDecrement(fElements2_HP_PM,fMaxLevelRefinements,true);
//	if(counter<nelemsamedim*0.5) {
		if(fElements2_PP.NElements())
			UniformPRefinementSelectionIncrement(fElements2_PP,fMaxPRefinements);
		...
		//		UniformPRefinementSelectionIncrement(coarsed,fMaxPRefinements);
		}
		if(fElements2_HM.NElements()) {
//		UniformPRefinementSelectionIncrement(estimator->fElements2_HM,fMaxPRefinements);
			TPZStack<int64_t> coarsed;
		...
		if(fElements2_HP.NElements())
			UniformHRefineSelection(fElements2_HP,fMaxLevelRefinements,true);
	//}
	if (fGDFEM) {
...
void TAdaptive::UniformHRefineSelection(TPZStack<int64_t>& indexfathers,int maxhref,bool neigh) {
	if (!fCMesh) {
		return;
	}
	fCMesh->Block().SetMatrix(&(fCMesh->Solution()));
...
void TAdaptive::UniformPRefinementSelectionIncrement(TPZStack<int64_t>& indextoprefine,int MaxPRefine) {
